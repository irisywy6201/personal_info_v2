<?php namespace John\ApiGuard\Controllers;

use App;
use John\ApiGuard\Models\ApiLog;
use Controller;
use Input;
use Route;
use Config;
    ...
    public function __construct()
    {
        $this->beforeFilter(function () {
            ...
            if ($keyAuthentication === true) {
                ...
                if (empty($this->apiKey) || $this->apiKey->exists() == false || $this->apiKey->deleted_at != NULL) {
                    // ApiKey not found
                    return $this->response->errorUnauthorized();
                }
            ...
            if ( ! empty($apiMethods[$method]['limits'])) {
                ...
                if (Config::get('api-guard::logging') === false) {
                    Log::warning("[John/ApiGuard] You specified a limit in the $method method but API logging needs to be enabled in the configuration for this to work.");
                }
                ...
                if ($this->apiKey != null && ! empty($limits['key'])) {
                    ...
                    if ($keyLimit == 0 || is_integer($keyLimit) == false) {
                        Log::warning("[John/ApiGuard] You defined a key limit to the " . Route::currentRouteAction() . " route but you did not set a valid number for the limit variable.");
                    } else {
                        if ( ! $this->apiKey->ignore_limits) {
                            ...
                            if ($keyIncrementTime == false) {
                                Log::warning("[John/ApiGuard] You have specified an invalid key increment time. This value can be any value accepted by PHP's strtotime() method");
                            } else {
                                ...
                                if ($apiLogCount >= $keyLimit) {
                                    Log::warning("[John/ApiGuard] The API key ID#{$this->apiKey->id} has reached the limit of {$keyLimit} in the following route: " . Route::currentRouteAction());
                ...
                if ( ! empty($limits['method'])) {
                    ...
                    if ($methodLimit == 0 || is_integer($methodLimit) == false) {
                        Log::warning("[John/ApiGuard] You defined a method limit to the " . Route::currentRouteAction() . " route but you did not set a valid number for the limit variable.");
                    } else {
                        if ($this->apiKey != null && $this->apiKey->ignore_limits == true) {
                            ...
                            if ($methodIncrementTime == false) {
                                Log::warning("[John/ApiGuard] You have specified an invalid method increment time. This value can be any value accepted by PHP's strtotime() method");
                            } else {
                                ...
                                if ($apiLogCount >= $methodLimit) {
                                    Log::warning("[John/ApiGuard] The API has reached the method limit of {$methodLimit} in the following route: " . Route::currentRouteAction());
