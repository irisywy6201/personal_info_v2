<div class="tab-content">
			...
			<tbody>
				<input  type="hidden"  name="{{$j=1}}" />
					@foreach($name as $downloads)
						@if($downloads->category==2)
							@if($downloads->isdelete==0)
							<tr>
								<td>
									<div class="col-sm-1">
										<button type="button" class="btn btn-info" data-toggle="collapse" data-target="#down2{{$j}}">+</button>
									</div>
									<div class="col-sm-10">
									...
									<div id="down2{{$j}}" class="collapse">
										...
										<div class="row" style="width:90%;margin-left:15px;">
											<div class="col-sm-4" style="">
												...
												<a id="connect2{{$j}}" href="#"><img src="/img/download.png" style="width:250px;"></a><br><br>
												<p id="name2{{$j}}" style="color:#fff;border-radius:4px;background-color:#993333; letter-spacing:1px;text-align:center;"></p>
												...
												<div class="dropdown" style="display:inline;">
													...
													@foreach($software_version as $software_versions)
														@if($software_versions->software_list_id==$downloads->id)
															<li onclick="get_connect2('{{ $software_versions->link }}','{{ $software_versions->name_zh }}','{{$j}}')"><a>{{ $software_versions->name_zh }}</a></li>
  ...
  <script>
  	function get_connect2(connection,name,j){
  		
  		 document.getElementById('connect2'+j).href=connection;
  		  document.getElementById('name2'+j).innerHTML='點擊上面圖片下載 '+name;
